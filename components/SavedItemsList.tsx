      ...
      <div className="flex flex-col gap-4 mb-6">
        <div className="flex items-center justify-between">
          <h3 className="text-xl font-bold text-white flex items-center gap-2">
            Action List
            <span className="text-sm font-normal text-zinc-500 bg-zinc-800 px-2 py-0.5 rounded-full">{filteredItems.length}</span>
          </h3>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="text-xs text-zinc-400 hover:text-white flex items-center gap-1 px-3 py-1.5 rounded bg-zinc-800 hover:bg-zinc-700 transition-colors disabled:opacity-50"
            ...
            {confirmClear ? (
              <div className="flex items-center gap-2 bg-red-500/10 px-2 py-1 rounded border border-red-500/20">
                ...
                <button onClick={onClear} className="text-xs font-bold text-red-500 hover:underline">Yes</button>
                <button onClick={() => setConfirmClear(false)} className="text-xs text-zinc-400 hover:text-white">No</button>
              </div>
            ) : (
              <button
                onClick={() => setConfirmClear(true)}
                className="text-xs text-red-400 hover:text-red-300 flex items-center gap-1 px-3 py-1.5 rounded hover:bg-red-500/10 transition-colors"
              >
          ...
          return (
            <div
              ...
              className={`
                    ...
                    ${isSelected ? 'border-primary ring-1 ring-primary/50 bg-zinc-800/50' : 'border-zinc-800 hover:border-zinc-700'}
                `}
              ...
              <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1.5">
                      ...
                      <span className="text-[10px] text-zinc-600">• {new Date(item.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                      {item.favorite && <Star className="w-3 h-3 text-yellow-500 fill-yellow-500 ml-auto mr-12 sm:mr-0 sm:ml-2" />}
                    </div>

                    <h4 className="font-medium text-zinc-200 mb-1 line-clamp-1 text-sm sm:text-base leading-snug">
                      {item.title}
                    </h4>
                    ...
                    {item.note && (
                      <div className="bg-yellow-500/10 border border-yellow-500/20 p-2.5 rounded-lg mt-2 relative">
                        <div className="absolute top-0 left-0 w-1 h-full bg-yellow-500/30 rounded-l-lg" />
                        <p className="text-sm text-yellow-200/90 pl-2 leading-relaxed">{item.note}</p>
                      </div>
                    )}
                    {item.summary && (
                      <div className="bg-purple-500/10 border border-purple-500/20 p-2.5 rounded-lg mt-2 relative">
                        <div className="absolute top-0 left-0 w-1 h-full bg-purple-500/30 rounded-l-lg" />
                        <p className="text-sm text-purple-200/90 italic pl-2 leading-relaxed">{item.summary}</p>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex flex-col gap-2 shrink-0">
                    ...
                    <button
                      ...
                      className={`p-2 rounded-lg transition-colors border ${item.favorite ? 'bg-yellow-500/10 text-yellow-500 border-yellow-500/20' : 'bg-zinc-900 text-zinc-600 border-zinc-800 hover:text-yellow-500 hover:bg-zinc-800'}`}
